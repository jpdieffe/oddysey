...
class App {
  ...
  async boot(): Promise<void> {
    const savedStage = Number.parseInt(localStorage.getItem('odyssey-campaign-stage') ?? '0', 10);
  ...
  private showLoading(): void {
    ...
    this.ui.appendChild(
      el('div', { class: 'screen' },
        el('div', { class: 'stack' },
          el('div', { class: 'title' }, el('h1', {}, 'ODYSSEY')),
    ...
    renderTitle(this.ui, {
      ...
      onResetCampaign: () => {
        if (this.setup.mapId === 0) {
          toast('The voyage is already at Book 1.');
          return;
        }
        if (!window.confirm('Restart the voyage and return to Book 1?')) return;
        this.setup.mapId = 0;
        localStorage.setItem('odyssey-campaign-stage', '0');
        this.showTitle();
        toast('The voyage has restarted at Book 1.');
      },
      onHelp: () => {
        this.screen = 'help';
  ...
  private advanceCampaign(multiplayer: boolean): void {
    ...
    localStorage.setItem('odyssey-campaign-stage', String(next));
...
async function updateToLatestBuild(): Promise<boolean> {
  try {
    ...
    if (!version.commit || version.commit === __BUILD_COMMIT__) {
      sessionStorage.removeItem('odyssey-update-attempt');
      return false;
    }

    // Avoid a reload loop if GitHub Pages briefly serves files from two deploys.
    if (sessionStorage.getItem('odyssey-update-attempt') === version.commit) return false;
    sessionStorage.setItem('odyssey-update-attempt', version.commit);
